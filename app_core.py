...
def crear_word_premium(nombre_archivo, tipo_reporte, datos):
    ...
    carpeta_destino = os.path.join(os.getcwd(), "Reportes")
...
# =====================================================================
# 2. MOTORES PREMIUM DE EXPORTACIÓN DE EXONERACIÓN (RRD)
# =====================================================================
def crear_word_desestimacion_rrd(nombre_archivo, datos):
    doc = Document()
    for section in doc.sections:
        section.top_margin = Inches(0.8)
        section.bottom_margin = Inches(0.8)
    ...
    titulo = doc.add_heading("REPORTE DE RIESGO DESESTIMADO (RRD)", level=0)
    for run in titulo.runs: 
        run.font.color.rgb = RGBColor(71, 85, 105)
        run.font.size = Pt(15)
        
    doc.add_paragraph(f"Expediente: {datos['expediente']}   |   Estatus: {datos['grado_txt']}").alignment = WD_ALIGN_PARAGRAPH.CENTER
    
    doc.add_heading('1. DATOS DEL CLIENTE EVALUADO', level=1)
    ...
    doc.add_heading('2. DICTAMEN DE EXONERACIÓN INTERNA (CNBV)', level=1)
    doc.add_paragraph(datos['resumen'])
    doc.add_paragraph(datos['observaciones_analista'])
    doc.add_paragraph(f"\n_____________________________\nFirma de Cierre Analista: {datos['analista']}")

    carpeta_des = os.path.join(os.getcwd(), "Desestimaciones")
    if not os.path.exists(carpeta_des): 
        os.makedirs(carpeta_des)
    doc.save(os.path.join(carpeta_des, nombre_archivo))

def crear_pdf_desestimacion_rrd(nombre_archivo, datos):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font('helvetica', 'B', 13)
    pdf.set_text_color(71, 85, 105)
    pdf.cell(0, 10, "REPORTE DE RIESGO DESESTIMADO (RRD)", 0, 1, 'C')
    pdf.set_font('helvetica', '', 9)
    pdf.set_text_color(0, 0, 0)
    pdf.cell(0, 5, f"Expediente: {datos['expediente']} | Grado: {datos['grado_txt']}", 0, 1, 'C')
    pdf.ln(5)
    resumen_l = str(datos['resumen']).replace('•', '-')
    obs_l = str(datos['observaciones_analista']).replace('•', '-')
    pdf.multi_cell(0, 5, f"Justificación de Cierre:\n{resumen_l}\n\nEvidencia y Notas de Campo:\n{obs_l}")
    pdf.ln(10)
    pdf.cell(0, 5, '___________________________', 0, 1, 'L')
    pdf.cell(0, 5, f"Firma de Exoneración Oficial: {datos['analista']}", 0, 1, 'L')
    
    carpeta_des = os.path.join(os.getcwd(), "Desestimaciones")
    if not os.path.exists(carpeta_des): 
        os.makedirs(carpeta_des)
    pdf.output(os.path.join(carpeta_des, nombre_archivo))
...
def registrar_en_bitacora_excel(metadata, id_empresa, giro, tipologia, accion, nombre_archivo="N/A", subcarpeta="N/A"):
    ...
    carpeta_registros = os.path.join(os.getcwd(), "Registros_PLD")
